...
def q3(name_score_dict):
    ...
    class Student:
        def __init__(self, a, b):
            self.name = a
            self.score = b
    for key, val in name_score_dict.items():
        stu = Student(key, val)
        res.append(stu)
...
class Layout:
    ...
    def recover_layoutText(self):
        ...
        """
        ret = [[" " for i in range(self.width)] for j in range(self.height)]
        for x in self.walls:
            ret[x[0]][x[1]] = '%'
        for x in self.foods:
            ret[x[0]][x[1]] = '.'
        ret[self.pacman_pos[0]][self.pacman_pos[1]] = 'P'
        temp = [''.join(i) for i in ret]
        ntemp = '\n'.join(temp)
        return ntemp
