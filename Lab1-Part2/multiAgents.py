    ...
    def getAction(self, gameState):
        ...
        def maximizer(state, depth, index_of_agent):
            ...
            for action in Actions:
                ...
                newValue = minimizer(newState, depth, index_of_agent + 1)[0]
                if newValue > value:
                    maxiAction = action
                    value = newValue
        ...
        def minimizer(state, depth, index_of_agent):
            ...
            for action in Actions:
                ...
                if index_of_agent == numAgents - 1:
                    newValue = maximizer(newState, depth - 1, 0)[0]
                else:
                    newValue = minimizer(newState, depth, index_of_agent + 1)[0]
                if newValue < value:
                    miniAction = action
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def maximizer(state, depth, index_of_agent, alpha, beta):
            ...
            for action in Actions:
                newState = state.generateSuccessor(index_of_agent, action)
                newValue = minimizer(newState, depth, index_of_agent + 1, alpha, beta)[0]
                if newValue > value:
                    value = newValue
                    maxiAction = action
                if value > beta:
        ...
        def minimizer(state, depth, index_of_agent, alpha, beta):
            ...
            for action in Actions:
                ...
                if index_of_agent == numAgent - 1:
                    newValue = maximizer(newState, depth - 1, 0, alpha, beta)[0]
                else:
                    newValue = minimizer(newState, depth, index_of_agent + 1, alpha, beta)[0]
                if newValue < value:
                    value = newValue
...
class MCTSAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState, mcts_time_limit=10):
        ...
        def Selection(cgs, cgstree):
            ...
            '''
            legal_actions = cgs.getLegalActions(0)
            if len(legal_actions) == 0:
                return (cgs, cgstree)
            if "North" in legal_actions and cgstree.north is None:
                return (cgs, cgstree)
            if "South" in legal_actions and cgstree.south is None:
                return (cgs, cgstree)
            if "East" in legal_actions and cgstree.east is None:
                return (cgs, cgstree)
            if "West" in legal_actions and cgstree.west is None:
                return (cgs, cgstree)
            if "Stop" in legal_actions and cgstree.stop is None:
                return (cgs, cgstree)
            children = []
            ...
            ngs, new_action = best_UCT(children)
            if new_action == "North":
                ngstree = cgstree.north
            elif new_action == "South":
                ngstree = cgstree.south
            elif new_action == "East":
                ngstree = cgstree.east
            elif new_action == "West":
                ngstree = cgstree.west
            else:
                ngstree = cgstree.stop
            return Selection(ngs, ngstree)
