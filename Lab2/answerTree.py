...
def entropy(Y: np.ndarray):
    ...
    for i in range(len(labels)):
        result -= counts[i] / n * np.log2(counts[i] / n)
    return result
...
def gain(X: np.ndarray, Y: np.ndarray, idx: int):
    ...
    for i in range (len(ufeat)):
        subset = np.array([Y[j] for j in range(X.shape[0]) if feat[j] == ufeat[i]])
        result -= featp[i] * entropy(subset)
    return result
    # raise NotImplementedError
...
def buildTree(X: np.ndarray, Y: np.ndarray, unused: List[int], depth: int, purity_bound: float, gainfunc: Callable, prefixstr=""):
    ...
    # TODO: YOUR CODE HERE
    for key in ufeat:
        subsetX = np.array([X[i] for i in range(X.shape[0]) if feat[i] == key])
        subsetY = np.array([Y[i] for i in range(Y.shape[0]) if feat[i] == key])
        root.children[key] = buildTree(subsetX, subsetY, unused, depth - 1, purity_bound, gainfunc)
    return root
    # raise NotImplementedError
    # return root
