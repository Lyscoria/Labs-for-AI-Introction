...
class NaiveBayesModel:
    ...
    def count(self):
        ...
        for text, label in self.dataset:
            ...
            for token in text:
                if (token in have_counted):
                    continue
                have_counted.append(token)
    ...
    def __call__(self, text):
        ...
        alpha = 1
        eps = 1e-6
        total_num = self.pos_neg_num[0] + self.pos_neg_num[1]
        predict_pos = self.pos_neg_num[0] / total_num
        predict_neg = self.pos_neg_num[1] / total_num
        for token in text:
            predict_pos *= (self.token_num[0].get(token, 0) + alpha) / (self.pos_neg_num[0] + alpha * self.V)
            predict_neg *= (self.token_num[1].get(token, 0) + alpha) / (self.pos_neg_num[1] + alpha * self.V)
...
class QAModel():
    ...
    def __call__(self, query):
        ...
        max_tfidf = 0
        choosen_document = dict()
        for document in self.document_list:
            tfidf_sum = 0
            for word in query:
                tfidf_sum += self.tfidf(word, document)
            if tfidf_sum > max_tfidf:
                max_tfidf = tfidf_sum
                choosen_document = document
        
        sentences = choosen_document['sentences']
        choosen_sentence = []
        max_idf = 0
        max_density = 0
        for sentence in sentences:
            ...
            density = term_num / len(sentence[0])
            if idf_sum > max_idf:
                max_idf = idf_sum
                max_density = density
                choosen_sentence = sentence
            elif idf_sum == max_idf and density > max_density:
                max_idf = idf_sum
                max_density = density
                choosen_sentence = sentence                
        return choosen_sentence[1]
        # raise NotImplementedError
