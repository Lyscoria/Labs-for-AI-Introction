...
class Attention(Node):
    ...
    def cal(self, X):
        ...
        softmax_score = softmax(E)
        output = softmax_score @ V + X
...
class ResLinear(Node):
    ...
    def cal(self, X):
        ...
        # 提示：相比于线性层，增加了残差连接
        # 即Linear(X)+X
        ret = X @ self.params[0] + self.params[1] + X
        self.cache.append(X)
        return ret
        # raise NotImplementedError
...
class LogSoftmax(Node):
    ...
    def cal(self, X):
        ...
        expX = np.exp(X)
        ret = np.log(EPS + expX / expX.sum(axis=self.dim, keepdims=True))
